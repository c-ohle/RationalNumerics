  ...
  public partial class CalculatorView : UserControl
  {
    ...
    string? parse()
    {
      ...
      try
      {
        ...
        if (btn_dec.Text[0] == 'C')
        {
          var t1 = MathRexp.GetContinuedFraction(r);
          var t2 = MathRexp.ParseContinuedFraction(t1); Debug.Assert(r == t2); //cross check
          return t1;
        }
    ...
    rat parse(int a, int b)
    {
      ...
      if (a == b)
        switch (h)
        {
          ...
          default:
            if (h[0] == '[') return MathRexp.ParseContinuedFraction(h);
            return rat.Parse(h);
        }
      for (int l = 0; l < 3; l++)
    ...
    double parsed(int a, int b)
    {
      ...
      if (a == b)
        switch (h)
        {
          ...
          case "℮": return Math.E;
          default:
            if (h[0] == '[') return (double)MathRexp.ParseContinuedFraction(h);
