...
{
  /// <summary>
  /// A Vector3 class based on <see cref="BigRational"/>.<br/>
  ...
  {
    public readonly BigRational X, Y, Z;
    public Vector3R(BigRational x, BigRational y, BigRational z)
    {
      X = x; Y = y; Z = z;
    }
    ...
    {
      return new Vector3R(BigRational.ReadFromBytes(ref rs), BigRational.ReadFromBytes(ref rs), BigRational.ReadFromBytes(ref rs));
    }
    ...
    public static Vector3R operator *(in Vector3R a, BigRational b)
    {
    ...
    public static int PtInline(in Vector3R a, in Vector3R b, in Vector3R c)
    {
      ...
      var cpu = BigRational.task_cpu; cpu.cross(a, b, c);
